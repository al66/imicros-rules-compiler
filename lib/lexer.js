...
class Lexer {
    ...
    _getNextToken () {
        while (this._peek()) {
            ...
            if (token.value == "#") {
                this._advance();
                return new Token({type: Token.LABEL, value: token.value, match: token});
            }
            if (token.type == "name") {
            ...
            if (token.type == "date") {
                ...
                return new Token({type: Token.DATE, value: Date.parse(date), str: date, match: token});                
            }
            if (token.type == "time") {
                this._advance();
                let d = new Date(0);
                ...
                return new Token({type: Token.TIME, value: t, str: token.value, match: token});                
            }
            if (token.type == "label") {
                this._advance();
                return new Token({type: Token.LABEL, value: token.value, match: token});
