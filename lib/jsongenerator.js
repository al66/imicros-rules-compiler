...
class JsonGenerator {
    ...
    __RULESET (node) {
        ...
        node.definitions.forEach(element => {
            ...
            node.init.forEach(assign => {
                if (assign.var && assign.var.name === name ) {
    ...
    __AND (node, rule) {
        ...
        if (node.interval) {
            ...
            exp = this._build(node.left, rule)+" & "+this._build(node.right, rule);
        }
        if (node.is) {
            let condition = {
    ...
    __OR (node, rule) {
        // OR left right
        let exp = this._build(node.left, rule)+", "+this._build(node.right, rule);
        if (node.is) {
            let condition = {
                input: node.input.name,
                expression: exp
