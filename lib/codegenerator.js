...
class Codegenerator {
    ...
    __RULE (node) {
        // RULE when then
        switch (this.hitpolicy) {
            case "C":
                return (this._build(node.when)+"{"+this._build(node.then)+";}");
            default:
    ...
    __VAR (node) {
        ...
        if (node.type) {
            ...
            /*
            if (node.type.node === Node.DATE_TYPE) {
                return ("Date.parse(getValue(c,'" + node.name + "',"+isArray+"))");
            }
            */
        }
    ...
    __BINARY (node) {
        ...
        if (node.left.type.node == Node.DATE_TYPE ) {
            return ("(new Date(new Date("+this._build(node.left)+").getTime() "+node.operator+" "+this._build(node.right)+"))");
        //} else if (node.right.node == Node.TIME) {
        } else if (node.left.type.node == Node.TIME_TYPE) {
            return ("(getTime("+this._build(node.left)+") "+node.operator+" "+this._build(node.right)+")");
        } else {
