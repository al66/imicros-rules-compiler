...
class Node {
    ...
    static get OBJECT_TYPE() { return "OBJECT_TYPE"; }
...
class Parser {
    ...
    _parseType() {
        //
        // type            : (..)? string | number | date | time | boolean | object
        //
        let isArray = false;
        ...
        if (this.lexer.current.type == Token.NAME) {
            ...
            if (this.lexer.current.value == "object") {
                this.lexer.eat(Token.NAME);
                return new Node({node: Node.OBJECT_TYPE, array: isArray, buildIn: "object"});
            }
        }
    }
